...
natoms_worked = 10;
natoms_worked = 280; %Папка 5x7 35 + 2 10NNN  
tstep = 2 * 1e-15; %m/s
mean(M,1)


start1 = 1;
mean_Vx = 1000*cumsum(M(start1:end,cVx)/natoms_in_free_mols)./((1:size(M,1)-start1+1)');
int_Vx_dt = 1000*tstep*1e9*cumsum(M(start1:end,cVx)/natoms_in_free_mols);
